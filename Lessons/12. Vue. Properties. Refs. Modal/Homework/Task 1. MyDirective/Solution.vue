    ...
    directives: {
        replace: {
            update: (element, bindings) => {
                const textWithoutSpans = element.innerHTML
                    .replaceAll('<span>', '')
                    .replaceAll('</span>', '');

                const stringToReplace = '<span>' + bindings.arg + '</span>';
                element.innerHTML = textWithoutSpans.replaceAll(bindings.arg, stringToReplace);
            },
        },
    },
    methods: {
